...
public class MaterialService {
    ...
    public List<MaterialVo4Unit> select(String materialParam, String color, String weight, String expiryNum,
                                        ...
                                        String remark, String categoryId, String mpList, int offset, int rows, String oem, String vin, String carModelCode)
            throws Exception {
    ...
    public Long countMaterial(String materialParam, String color, String weight, String expiryNum,
                              String enableSerialNumber, String enableBatchNumber, String enabled,
                              String remark, String categoryId, String mpList, String oem, String vin, String carModelCode) throws Exception {
        Long result = null;
        try {
            List<Long> idList = new ArrayList<>();
            if (StringUtil.isNotEmpty(categoryId)) {
            ...
            result = materialMapperEx.countsByMaterial(materialParam, color, weight, expiryNum,
                    enableSerialNumber, enableBatchNumber, enabled, remark, idList, mpList, oem, vin, carModelCode);
        } catch (Exception e) {
            JshException.readFail(logger, e);
        }
        return result;
    ...
    public int checkIsExist(Long id, String name, String model, String color, String standard, String mfrs,
                            String otherField1, String otherField2, String otherField3, String unit, Long unitId, String oem, String vin, String carModelCode) throws Exception {
        return materialMapperEx.checkIsExist(id, name, model, color, standard, mfrs, otherField1,
                otherField2, otherField3, unit, unitId, oem, vin, carModelCode);
    }
    ...
    public List<MaterialVo4Unit> exportExcel(String materialParam, String color, String weight, String expiryNum, String enabled,
                                             String enableSerialNumber, String enableBatchNumber, String remark, String categoryId, String oem, String vin, String carModelCode) throws Exception {
        List<MaterialVo4Unit> resList = new ArrayList<>();
        List<MaterialVo4Unit> list = null;
        try {
            List<Long> idList = new ArrayList<>();
            ...
            list = materialMapperEx.exportExcel(materialParam, color, weight, expiryNum, enabled, enableSerialNumber,
                    enableBatchNumber, remark, idList, oem, vin, carModelCode);
        } catch (Exception e) {
            JshException.readFail(logger, e);
        }
        if (null != list) {
    ...
    public List<MaterialVo4Unit> getListWithStock(List<Long> depotList, List<Long> idList, String materialParam, Integer zeroStock,
                                                  String column, String order, Integer offset, Integer rows, String oem, String vin, String carModelCode) throws Exception {
        ...
        List<MaterialVo4Unit> dataList = materialMapperEx.getListWithStock(depotList, idList, materialParam, zeroStock, column, order, offset, rows, oem, vin, carModelCode);

        Map<Long, Depot> depotMap = queryDepot(dataList);
        Map<Long, List<MaterialCurrentStock>> currentStockPrice = getCurrentStockPrice(dataList);
        for (MaterialVo4Unit item : dataList) {
            ...
            List<MaterialCurrentStock> materialCurrentStocks = currentStockPrice.get(item.getId());
            BigDecimal count = BigDecimal.ZERO;
            if (materialCurrentStocks != null && !materialCurrentStocks.isEmpty()) {
                for (MaterialCurrentStock materialCurrentStock : materialCurrentStocks) {
                    if (materialCurrentStock.getDepotId().equals(item.getDepotId())) {
                        count = count.add(materialCurrentStock.getCurrentNumber());
                    }
                }
            }
            Depot depot = depotMap.get(item.getDepotId());
            if (depot != null) {
                item.setDepotName(depot.getName());
            }
            item.setCurrentStock(count);
            item.setCurrentStockPrice(item.getPurchaseDecimal().multiply(count));
        }
        if (zeroStock == 0) {
            dataList.removeIf(materialVo4Unit -> BigDecimal.ZERO.compareTo(materialVo4Unit.getCurrentStock()) == 0);
        }
        return dataList;
    }


    public Map<Long, Depot> queryDepot(List<MaterialVo4Unit> dataList){
        if (dataList == null || dataList.isEmpty()) {
            return new HashMap<>();
        }
        List<Long> ids = dataList.stream().map(MaterialVo4Unit::getDepotId).collect(Collectors.toList());
        if (ids.isEmpty()) {
            return new HashMap<>();
        }

        List<Depot> depots = depotMapper.queryByIds(ids);
        return depots.stream().collect(Collectors.toMap(Depot::getId, v->v, (k1,k2)->k1));
    }



    private Map<Long, List<MaterialCurrentStock>> getCurrentStockPrice(List<MaterialVo4Unit> dataList) {
        if (dataList == null || dataList.isEmpty()) {
            return new HashMap<>();
        }
        List<Long> ids = dataList.stream().map(Material::getId).collect(Collectors.toList());
        List<MaterialCurrentStock> materialCurrentStocks = materialCurrentStockMapper.queryByIds(ids);
        if (materialCurrentStocks == null || materialCurrentStocks.isEmpty()) {
            return new HashMap<>();
        }

        return materialCurrentStocks.stream().collect(Collectors.groupingBy(MaterialCurrentStock::getMaterialId));
    }


    public int getListWithStockCount(List<Long> depotList, List<Long> idList, String materialParam, Integer zeroStock, String oem, String vin, String carModelCode) {
        return materialMapperEx.getListWithStockCount(depotList, idList, materialParam, zeroStock, oem, vin, carModelCode);
    }
